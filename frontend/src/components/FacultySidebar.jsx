        ...
        {items.map((item) => {
          ...
          return (
            <button
              ...
              className={`relative group flex w-full items-center gap-3 rounded-xl px-3 py-2 text-left text-sm font-medium transition-all duration-200 ${isActive
                  ? "text-white"
            ...
            >
              {isActive && (
                <motion.div
                  layoutId="active-faculty-sidebar-bg"
                  className="absolute inset-0 bg-gradient-to-r from-brand-ink to-brand-ocean rounded-xl z-0 shadow-md"
                  transition={{ type: "spring", stiffness: 380, damping: 30 }}
                />
              )}
              <span className={`relative z-10 transition ${isActive ? "text-white" : "text-brand-ocean group-hover:text-brand-ink"}`}>{MENU_ICONS[item]}</span>
